...
async function startCompetition(){if(!state.compRoom)return;const btn=$("#startCompetitionTestBtn");busy(btn,true,"loading passage...");const arena=await createArena("#competitionArenaMount",{type:"competition",sourceId:state.compRoom},{page:"competition-room",roomId:state.compRoom,type:"competition"});busy(btn,false);if(arena){$("#competitionArenaIntro")?.classList.add("hidden");$("#competitionArenaMount")?.classList.add("active");setTimeout(()=>arena.focus(),120);}}
