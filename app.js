...
function syncSettingsUI(){
  ...
  $("#togglePunctuation")?.classList.toggle("active",s.punctuation);$("#toggleNumbers")?.classList.toggle("active",s.numbers);const lang=LANGUAGES.find(x=>x[0]===s.language);$("#activeLanguageLabel").textContent=lang?lang[1]:s.language;
...
function fillLanguages(){
  const html=LANGUAGES.map(([v,n,en])=>`<option value="${v}">${esc(n)}${en!==n?` — ${esc(en)}`:""}</option>`).join("");
...
function renderLanguageGrid(q=""){
  const g=$("#languageGrid");if(!g)return;const query=q.trim().toLowerCase();
  const items=LANGUAGES.filter(([v,n,en])=>!query||`${v} ${n} ${en}`.toLowerCase().includes(query));
...
function setAvatar(el,user){if(!el||!user)return;el.innerHTML="";if(user.avatarData){const img=document.createElement("img");img.src=user.avatarData;img.alt=`${user.displayName} profile photo`;el.appendChild(img);el.classList.add("has-photo");}else{el.textContent=initials(user.displayName);el.classList.remove("has-photo");}}
...
function routeFromHash(){const m=location.hash.match(/^#competition\/([A-Z0-9]{6})$/i);return m?{page:"competition-room",code:m[1].toUpperCase()}:null;}
async function restoreRoute(){const route=routeFromHash();if(route&&state.user){try{await joinRoom("competition",route.code,{silent:true,replaceHash:true});return true;}catch(e){history.replaceState(null,"",location.pathname+location.search);toast(e.message,"error");}}return false;}
...
async function compressAvatar(file){
  if(!file||!/^image\/(png|jpeg|webp)$/.test(file.type))throw new Error("Use PNG, JPG, or WebP.");if(file.size>5*1024*1024)throw new Error("Image is too large. Maximum 5 MB.");
  const url=URL.createObjectURL(file);try{const img=await new Promise((resolve,reject)=>{const im=new Image();im.onload=()=>resolve(im);im.onerror=()=>reject(new Error("Image could not be read."));im.src=url;});const size=160,canvas=document.createElement("canvas");canvas.width=size;canvas.height=size;const ctx=canvas.getContext("2d");const side=Math.min(img.naturalWidth,img.naturalHeight),sx=(img.naturalWidth-side)/2,sy=(img.naturalHeight-side)/2;ctx.drawImage(img,sx,sy,side,side,0,0,size,size);return canvas.toDataURL("image/jpeg",.78);}finally{URL.revokeObjectURL(url);}}
async function saveAvatarFile(file){const btn=$("#uploadAvatarBtn");busy(btn,true,"saving…");try{const avatarData=await compressAvatar(file);const d=await api("updateAvatar",{avatarData},true);state.user=d.user;applyUser();toast("profile photo saved");}catch(e){toast(e.message,"error");}finally{busy(btn,false);$("#avatarFileInput").value="";}}
async function removeAvatar(){const btn=$("#removeAvatarBtn");busy(btn,true,"removing…");try{const d=await api("updateAvatar",{avatarData:""},true);state.user=d.user;applyUser();toast("profile photo removed");}catch(e){toast(e.message,"error");}finally{busy(btn,false);}}
...
async function createRoom(type){const comp=type==="competition",b=$(comp?"#createCompetitionBtn":"#createMultiBtn");busy(b,true,"creating…");try{const d=await api("createRoom",{type,name:$(comp?"#compName":"#multiName").value.trim(),visibility:$(comp?"#compVisibility":"#multiVisibility").value,maxPlayers:Number($(comp?"#compMax":"#multiMax").value),language:$(comp?"#compLanguage":"#multiLanguage").value,listSize:Number($(comp?"#compList":"#multiListSize").value),duration:Number($(comp?"#compDuration":"#multiDuration").value)},true);enterRoom(type,d.room);}catch(e){toast(e.message,"error");}finally{busy(b,false);}}
async function joinRoom(type,code,opts={}){const clean=String(code||"").trim().toUpperCase();if(!/^[A-Z0-9]{6}$/.test(clean)){const e=new Error("Room code must be 6 characters.");if(!opts.silent){toast(e.message,"error");return null;}throw e;}try{const d=await api("joinRoom",{type,code:clean},true);enterRoom(type,d.room,opts);return d;}catch(e){if(!opts.silent){toast(e.message,"error");return null;}throw e;}}
function enterRoom(type,room,opts={}){const id=room.roomId;if(type==="competition"){state.compRoom=id;clearInterval(state.compPoll);navigate("competition-room");if(!opts.replaceHash)history.pushState({roomCode:room.code},"",`#competition/${room.code}`);else history.replaceState({roomCode:room.code},"",`#competition/${room.code}`);state.compPoll=setInterval(()=>refreshRoom("competition"),POLL_MS);}else{state.multiRoom=id;$("#multiHome")?.classList.add("hidden");$("#multiRoom")?.classList.remove("hidden");clearInterval(state.multiPoll);state.multiPoll=setInterval(()=>refreshRoom("multiplayer"),POLL_MS);}refreshRoom(type);}
async function refreshRoom(type){const id=type==="competition"?state.compRoom:state.multiRoom;if(!id)return;try{const d=await api("roomState",{roomId:id},true),r=d.room,p=d.players||[];if(type==="competition"){$("#compRoomName").textContent=r.name;$("#compRoomCode").textContent=r.code;$("#compRoomStatus").textContent=(r.status||"open").toLowerCase();$("#compRoomLanguage").textContent=languageLabel(r.language);$("#compRoomDuration").textContent=`${r.duration}s`;$("#compRoomCount").textContent=`${p.length}/${r.maxPlayers}`;const host=p.find(x=>x.userId===r.hostUserId);$("#compRoomHost").textContent=host?.displayName||"host";$("#competitionPlayers").innerHTML=renderPlayers(p,false);const me=p.find(x=>x.userId===state.user.userId);$("#startCompetitionTestBtn").disabled=!!me?.finished||r.status==="FINISHED";if(r.code&&!location.hash.includes(r.code))history.replaceState({roomCode:r.code},"",`#competition/${r.code}`);}else{$("#multiRoomName").textContent=r.name;$("#multiRoomCode").textContent=r.code;$("#multiPlayers").innerHTML=renderPlayers(p,true);const me=p.find(x=>x.userId===state.user.userId),isHost=r.hostUserId===state.user.userId,allReady=p.length>=2&&p.every(x=>x.ready);$("#multiReadyBtn").textContent=me?.ready?"ready ✓":"i'm ready";$("#multiStartBtn").classList.toggle("hidden",!isHost||r.status!=="OPEN");$("#multiStartBtn").disabled=!allReady;$("#multiStartBtn").title=allReady?"Start the synchronized countdown":"All players, including the host, must be ready";if(r.status==="STARTED"&&r.startAt)scheduleRace(r);}}catch(e){console.warn(e);if(type==="competition"){clearInterval(state.compPoll);state.compRoom=null;history.replaceState(null,"",location.pathname+location.search);navigate("competition");toast(e.message,"error");}}}
function languageLabel(code){const l=LANGUAGES.find(x=>x[0]===code);return l?`${l[1]}${l[2]!==l[1]?` · ${l[2]}`:""}`:code||"—";}
function renderPlayers(p,race){return p.slice().sort((a,b)=>(b.finished-a.finished)||(b.wpm-a.wpm)||(b.progress-a.progress)).map((x,i)=>`<div class="player-row"><div class="player-rank">${i+1}</div><div class="player-name"><strong>${esc(x.displayName)}</strong><small>${race?(x.ready?"ready":"waiting"):(x.finished?"finished":"ready to type")}</small><div class="player-progress"><span style="width:${clamp(x.progress,0,100)}%"></span></div></div><div class="player-stat">${Math.round(x.wpm)} wpm</div><div class="player-stat">${Math.round(x.accuracy||0)}%</div></div>`).join("");}
async function startCompetition(){if(!state.compRoom)return;await createArena("#competitionArenaMount",{type:"competition",sourceId:state.compRoom},{page:"competition-room",roomId:state.compRoom,type:"competition"});$("#competitionArenaMount")?.scrollIntoView({behavior:"smooth",block:"center"});}
...
function wire(){
  ...
  window.addEventListener("popstate",async()=>{const route=routeFromHash();if(route){try{await joinRoom("competition",route.code,{silent:true,replaceHash:true});}catch{navigate("competition");}}else if(state.user)navigate("competition");});
