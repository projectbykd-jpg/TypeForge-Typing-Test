...
class Arena{
  ...
  async repeat(){const p=this.test.request||state.lastTestParams||currentMainParams();if(this.context.main)await createArena("#mainArenaMount",p,{page:"type",main:true});else if(this.context.roomId){const sel=this.context.type==="competition"?"#competitionArenaMount":"#multiArenaMount";await createArena(sel,p,this.context);}}
...
function roomRow(x,type){return `<div class="room-row"><div><h4>${esc(x.name)}</h4><p>${esc(x.language)} · ${x.listSize} · ${x.duration}s · ${x.playerCount}/${x.maxPlayers}</p></div><button class="ghost-button" data-room-code="${esc(x.code)}" data-room-kind="${type}">join</button></div>`;}
