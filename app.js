...
function setTypingFocus(on,context={}){document.body.classList.toggle("typing-focus",!!on);document.body.classList.toggle("typing-focus-main",!!on&&!!context.main);document.body.classList.toggle("typing-focus-room",!!on&&!!context.roomId);}
...
class Arena{
  ...
  async start(){if(this.started)return;this.started=true;setTypingFocus(true,this.context);this.startedPerf=performance.now();this.wordStart=this.startedPerf;$("#typingConfig")?.classList.add("dimmed");api("startTest",{testId:this.test.testId},true).catch(()=>{});this.timer=setInterval(()=>this.tick(),100);if(this.context.roomId)this.roomTimer=setInterval(()=>this.pushRoomProgress(),POLL_MS);}
  ...
  destroy(){clearInterval(this.timer);clearInterval(this.roomTimer);setTypingFocus(false);this.mount.innerHTML="";if(state.activeArena===this)state.activeArena=null;$("#typingConfig")?.classList.remove("dimmed");}
}
...
async function startCompetition(opts={}){
  ...
  if(state.competitionLoading)return state.activeArena||null;
  if(state.activeArena?.context?.type==="competition"&&state.activeArena?.context?.roomId===state.compRoom&&!state.activeArena.finished&&!opts.force){state.activeArena.focus();return state.activeArena;}
  state.competitionLoading=true;
  let btn=$("#startCompetitionTestBtn");busy(btn,true,"menyiapkan teks…");
  try{
    ...
    if(btn)btn.textContent="ulang attempt";
    setTimeout(()=>arena.focus(),120);return arena;
  }catch(e){
    $("#competitionArenaIntro")?.classList.remove("hidden");let mount=$("#competitionArenaMount");
    if(mount)mount.innerHTML=`<div class="arena-error"><strong>Competition belum bisa dimuat.</strong><span>${esc(e.message||"Coba lagi.")}</span><button class="accent-button" type="button" id="retryCompetitionArena">coba lagi</button></div>`;
    setTimeout(()=>{$("#retryCompetitionArena")?.addEventListener("click",()=>startCompetition({force:true}));},0);
    if(!opts.silent)toast(e.message||"Competition belum bisa dimuat.","error");return null;
...
function wire(){
  ...
  $("#startGlobalCompetitionBtn").onclick=startGlobalCompetition;if($("#quickStartCupBtn"))$("#quickStartCupBtn").onclick=()=>{navigate("competition");setTimeout(startGlobalCompetition,80);};$("#refreshGlobalCompetitionBtn").onclick=loadGlobalCompetition;$("#createCompetitionBtn").onclick=()=>createRoom("competition");$("#joinCompetitionBtn").onclick=()=>joinRoom("competition",$("#compJoinCode").value);$("#refreshCompetitionBtn").onclick=()=>{loadGlobalCompetition();loadCompetitionRooms();};$("#startCompetitionTestBtn").onclick=()=>startCompetition({force:true});$("#leaveCompetitionBtn").onclick=()=>leaveRoom("competition");$("#compRoomCode").onclick=e=>copyCode(e.currentTarget);if($("#compRoomCodeCopy"))$("#compRoomCodeCopy").onclick=e=>copyCode(e.currentTarget);
